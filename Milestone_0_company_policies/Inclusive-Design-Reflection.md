# Working with Vulnerable Populations - Rui Chosa

## Research & Learn
### Who are considered vulnerable populations, and what challenges might they face in digital spaces?
- **Who:** People aged under 18 or other individuals who may be unable to take care of themselves or are unable to protect themselves.
- **Challenges:** limited access to quality healthcare, inadequate/unsafe housing, social exclusion, lack of transportation, and, particularly during crises, high risks of exploitation, violence, and food insecurity. 

### What ethical considerations are important when designing or working with neurodivergent individuals? (Hint: Avoid overwhelming UX, respect sensory needs, ensure clear communication.)
- Making UI simple
- Modulate Environmental Stimuli
- Be Patient with Processing Time

### How can you make interactions and content more accessible for people with ADHD or Autism? (e.g., simple language, predictable navigation, reducing cognitive load)
- Use plain and clear language
- Reduce cognitive overload
- Ensure predictable navigation
- Minimize sensory overload

### How can we support neurodivergent team members in a professional setting? (Hint: Be clear in communication, respect different working styles, and allow flexibility.)
- Be clear in communication
- Be respectful and patient with them
- Provide sensory-friendly options

## Reflection
### How can you adjust your communication style to be more inclusive of neurodivergent users and teammates?
I can use simple language and be clear about everything. I can be respectful and patient with thme when they need time.

### What are some common UX or communication pitfalls that might make Focus Bear less accessible or supportive?
- Complicated UI
- Unclear language
- Lack of flexibility and userbility

### What is one practical change you can make in your work to better support vulnerable populations?
One practical change I can make is using clear and plain language. It conveys my thoughts and intentions clearly.

## Task
### Read a first-person account from someone with ADHD or Autism about their experiences with productivity tools. (Hint: Look for blog posts, videos, or community discussions.)
I read a first-person blog post by someone with ADHD about their experience using productivity tools. They explained that traditional productivity methods did not work for them, so they chose tools like planners and distraction-blocking apps that better suited how their brain works. This helped me understand that productivity systems should be flexible and adapted to individual needs.

<url="https://witanddelight.com/2025/02/work-routine-productivity-tools-for-adhd/">

### Identify one design or communication improvement that could make Focus Bear more accessible.
I feel like there are lots of features which is overwhelming. If this app is suppose to be for neurodivergent individuals, it should be more simple and accesible for the users.

### Practice writing a clear, patient, and supportive response to a hypothetical user struggling with executive functioning.
Let's slow it down. It's okay to struggle. You can take time. We can do step-by-step together. 